...
// TODO split arc|double arc by new node
namespace Dwader.Navigation
{
	...
	public class Graph
	{
        ...
        /// <summary>
        /// Creates a node, adds to the graph and returns its reference. Overload for double
        /// </summary>
        /// <param name="x">X coordinate.</param>
        /// <param name="y">Y coordinate.</param>
        /// <param name="z">Z coordinate.</param>
		...
		/// <summary>
        /// overload for ClosestNode
        /// </summary><see cref="ClosestNode(Point3D,out double,bool)"/>
		...
		{			
            return ClosestNode(new Point3D(PtX, PtY, PtZ), out Distance, IgnorePassableProperty);
		}
        /// <summary>
        ...
        /// <returns></returns>
        public Node NodeByName(string sname)
        {
            if(sname == null || sname == "") return null;
            foreach (Node N in LN)
            {
                if (N.Position.name == sname) return N;
            }
            return null;
        }
		...
		{
			return ClosestArc(new Point3D(PtX, PtY, PtZ), out Distance, IgnorePassableProperty);
		}
        /// <summary>
        /// Load Graph from binary format file 
        /// </summary>
        /// <param name="basefile"></param>
        /// <returns></returns>
        public static Graph LoadFromFile(string basefile)
        {
            using (Stream StreamRead = File.OpenRead(basefile))
            {
                BinaryFormatter BinaryRead = new BinaryFormatter();
                BinaryRead.AssemblyFormat = System.Runtime.Serialization.Formatters.FormatterAssemblyStyle.Simple;
        ...
        /// <param name="basefile">file name</param>
        public static void StoreToFile(Graph G, string basefile)
        {
            using (Stream StreamWrite = File.Create(basefile))
            {
                BinaryFormatter BinaryWrite = new BinaryFormatter();
                BinaryWrite.AssemblyFormat = System.Runtime.Serialization.Formatters.FormatterAssemblyStyle.Simple;
